...
class ErrorTest(TestCase):
    ...
    def make_parser(self):
        ...
        with TraceVariables(False):
            ...
            memberdef = BLine(typename & name) > tuple
            
            block = Delayed()
            
            line = (BLine(name) | block)
            
            BlockStart = Or(BLine(name & introduce),
                            BLine(name) ** make_error('BlockStart-OnlyName'),
                            BLine(introduce) ** make_error('BlockStart-OnlyColon'))
